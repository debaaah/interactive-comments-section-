            ...
            if (action === "plus") {
              if (!clicked[id].plus) {
                ...
                if (clicked[id].minus) {
                  setClicked({
                    ...clicked,
                    [id]: { plus: false, minus: false },
                  });
                } else {
                  setClicked({
                    ...clicked,
                    [id]: { plus: true, minus: false },
                  });
                }
              }
            } else if (action === "minus" && item.score !== 0) {
...
console.log('diveclassname', divClassName, 'displaystate', displayState, 'displaystateedit', displayStateEdit)
  ...
  const sentWho = (username, id = null, content = null) => {
    if (data.currentUser.username === username && id === null) {
      return <span className="blue">you</span>;
      ...
      return (
        <div className="edit-area">
          ...
          <div
            className="edit"
            onClick={() => settingDisplayState2(id, "displayed", content)}
          >
            <span>
      ...
      return (
        <div
          className="edit-area"
          onClick={() => settingDisplayState(id, "displayed", username)}
        >
          <div className="reply">
            <span>
    ...
    if (data.currentUser.username === username) {
      return (
        <div className="comment">
          <div className={divClassName[id]}>
            <span className="blueBold">{repliedUsername}</span>
            {newContent}
            <div style={style}></div>
          </div>
          <form
      ...
      <div id={id}>
        ...
        <div className="comment-box">
          <div className="score-box-box">
            <div className="score-box">
              <div onClick={() => editScore("plus", id)} className="plus">
              ...
              <div onClick={() => editScore("minus", id)} className="minus">
        ...
        {replies
          ? replies.map((item, index) =>
              reply(item, user.username, index2, index)
            )
          : null}
        ...
        return (
          <div id={id}>
            ...
            <div className="reply-box">
              {replies
                ? replies.map((item, index) =>
                    reply(item, user.username, index1, index)
                  )
                : null}
            </div>
