...
const Main = () => {
        ...
        return(
            <div id={id}>
                ...
                <div  className='reply-box'>
                    ...
                    {replies? replies.map((item, index) => (reply(item, parentUsername, parentIndex, index))) : null}
                </div>
    ...
    const ifReply = (replies, parentUsername, parentIndex) => {
        if (replies){
       // console.log('parent', parentUsername , parentIndex)
        return(
                <div>
                    {replies.map((item, index) => (reply(item, parentUsername, parentIndex, index)))}
                </div>
        )
        }
    }
  //  console.log(parentUsername.current)
    ...
    const editScore = (action, id) => {
     ...
     const sentWho = ( username, id, content ) => {
     ...
     //checks if the comment was made by the current user, if yes, makes the comment editable else, post the comment
      const sentWho2 = ( username, content, id, x, parents) => {
        if(parentUsername !== ''){parentUsername.current = parents}
    ...
    const editComment = (id) => {
     ...
     const reply = (item, parentUsername, parentIndex, index) => {
