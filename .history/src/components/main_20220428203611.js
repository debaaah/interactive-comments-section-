...
const Main = () => {
  ...
  /*  //topmost comment container, it is the first function called. it returns a div that calls other functions that handles/displays diffrent parts of the comment 
   const fullComment = (item, index1) => {
        //let index1 =index
  ...
  //  console.log(parentUsername.current)
    console.log(data)
    ...
    const editScore = (action, id) => {
     ...
     //checks if the comment was made by the current user, if yes, makes the comment editable else, post the comment
      const sentWho2 = ( username, content, id, x, parents) => {
          //console.log(newContent)
          //console.log(username, id, content, parents)
        if(parentUsername !== ''){
        ...
        if (x){      
            repliedUsername = '@' + parentUsername.current + ' '
    ...
    const editComment = (id) => {
     ...
     const reply = (item, parentUsername, parentIndex, index2) => {
       console.log('reply', parentUsername, parentIndex) 
        ...
        return(
            ...
            <div className='comment-box'>
                    ...
                    <div className='content-box'>
                        ...
                        {sentWho2(user.username, content, id, true, parentUsername)}
                    </div>
                </div>
                {replies? replies.map((item, index) => (reply(item, user.username, index2, index))) : null}
                {inputBox(id, user.username)}
            </div>
        )
    }
   ...
   //
    ...
    const settingDisplayState = (id, states, repliedUser) => {
       ...
       const replyForm = (id, repliedUser) => {
        const newInput = {
            content: inputValue,
            createdAt: Date.now(),
            id: latestId + 1,
        ...
        const replyReply = (item) => {
            item.map((item, index) => {
                    if(id === item.id){
                        data.comments[index].replies.push(newInput)
...
/*
   {comment(content, createdAt, id, score, user, false, '')}
                                (content, createdAt, id, score, user, x, parentUsername) => {
         //const newCreatedAt = createdAt 
*/
//////////////////////////////////////////////////